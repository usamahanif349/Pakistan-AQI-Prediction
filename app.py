...
<style>
    ...
    .stButton>button:hover {{ transform: translateY(-1px); color: {DARKTEXT[active]}; }}
...
# ===================== MAP =====================
    ...
    if map_df["lat"].notna().all():
        st.map(map_df.rename(columns={"lat": "latitude", "lon": "longitude"}), size=10000, color=A)
    else:
        st.warning("Some cities have no latitude or longitude in city_lookup.json, so only the table is shown.")

    st.dataframe(map_df.sort_values("Predicted AQI", ascending=False),
...
# ===================== HISTORICAL TREND =====================
    ...
    style_axes(ax, fig)
    ax.plot(city_hist["date"], city_hist["aqi_us"], color=A, linewidth=1.6)
